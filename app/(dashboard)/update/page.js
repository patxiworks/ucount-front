...
import { saveToLocalStorage, getFromLocalStorage, isOnline } from '@/utils/storageUtils';
...
const DynamicActivityForm = () => {
  ...
  useEffect(() => {
    const loadActivityData = async () => {
      ...
      const localData = getFromLocalStorage("formData");
      if (isOnline()) {
        const url = "http://127.0.0.1:8000/api/activity/ccp/events/8/participants/13/";
  ...
  const processOutbox = async () => {
    const outbox = getFromLocalStorage("outbox") || [];
    if (outbox.length > 0 && isOnline() && session?.accessToken) {
      ...
      if (successfullySent.every(Boolean)) {
        saveToLocalStorage("outbox", []); // Clear outbox if all items sent
      }
    }
  };
  ...
  const storeInOutbox = (data) => {
    const outbox = getFromLocalStorage("outbox") || [];
    outbox.push(data);
    saveToLocalStorage("outbox", outbox);
  }
  ...
  return (
    ...
    <form className="form-container">
      ...
      {/*<div className="date-selection">
        ...
        <label htmlFor="today" className="date-control">
          <input
              type="radio"
              id="today"
              name="dateType"
              value="today"
              defaultChecked
              onChange={() => handleDateTypeChange('today')}
            />
            <span>Today</span>
          </label>
          
          <label htmlFor="past" className="date-control">
            <input
              type="radio"
              id="past"
              name="dateType"
              value="past"
              onChange={() => handleDateTypeChange('past')}
            />
      ...
      <div className="date-box">
        ...
        {/*<input
          type={dateType === 'today' ? "text" : "date"}
          //value={selectedDate}
          value={dateType === 'today' ? todayDate : selectedDate}
          onChange={handleDateChange}
          min={dateType === 'today' ? todayDate : undefined}
          max={dateType === 'past' ? todayDate : undefined}
          disabled={!dateType}
          className="date-box"
        />*/}
      </div>
